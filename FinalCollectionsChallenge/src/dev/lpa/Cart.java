...
public class Cart {
    ...
    public void addItem(InventoryItem item, int qty) {//function to add item to the cart

        products.merge(item.getProduct().sku(), qty, Integer::sum);

        if (!item.reserveItem(qty)) {//here it checks the reserveItem list to make sure its available if not then below message is printed
            System.out.println("Ouch, something went wrong, could not add item");
        }
    }

    public void removeItem(InventoryItem item, int qty) {//function to remove the item form the cart.
    ...
    public void printSalesSlip(Map<String, InventoryItem> inventory) {
        ...
        for (var cartItem : products.entrySet()) {//this loops iterates through every item in the list with counting the item and adding the price in the total accordingly
    ...
    @Override
    public String toString() {//it gives the over all info of the cart
        return "Cart{" +
                "id=" + id +
                ", cartDate=" + cartDate +
                ", products=" + products +
